...
CURRENT_MODEL_ID_FROM_SERVER = "Dolphin3.0-Llama3.1-8B-Q8_0.gguf"
...
async def handle_greeting_response(user_english_input, call_manager, websocket):
    ...
    history.add_message(HumanMessage(content=user_english_input))
...
async def handle_conversation(user_english_input, call_manager, websocket):
    ...
    # print(f"////////////")
    messages = call_manager.prompt_template.format_messages(history=history.messages)
    try:
        llm_response = await llm.ainvoke(messages)
        print(f"[RAW LLM] {llm_response.content}")
